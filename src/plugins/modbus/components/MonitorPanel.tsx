    ...
    <div className={styles.panelPage}>
      <section className={styles.workbenchSection}>
        <div className={styles.panelHeading}>
          <div><strong>轮询监控</strong><span className={styles.hint}>把常用地址组织成监控表；慢请求完成后再安排下一周期，不积压、不重入。</span></div>
          ...
          <table className={styles.table}>
            ...
            <tbody>{rows.map(row => {
              ...
              return <tr key={row.id}>
                ...
                <td><select className="liquid-glass-input liquid-glass-select" value={format.value_type} disabled={request.functionCode <= 2} onChange={event => patchFormat(row, { value_type: event.target.value as ValueFormat["value_type"] })}>{["bool","uint16","int16","uint32","int32","float32","uint64","int64","float64","hex","binary","ascii","utf8"].map(item => <option key={item}>{item}</option>)}</select></td>
                <td><select className="liquid-glass-input liquid-glass-select" value={format.byte_order} disabled={request.functionCode <= 2} onChange={event => patchFormat(row, { byte_order: event.target.value as ValueFormat["byte_order"] })}>{["ABCD","BADC","CDAB","DCBA"].map(item => <option key={item}>{item}</option>)}</select></td>
