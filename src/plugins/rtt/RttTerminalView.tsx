...
import { mountXtermHost, type XtermHostLifecycle } from "../../components/Terminal/xtermHostLifecycle";
...
export default function RttTerminalView({ chunks, connected, onData }: Props) {
  ...
  useEffect(() => {
    ...
    if (!host || hostLifecycleRef.current) return;

    let input: { dispose(): void } | null = null;
    const lifecycle = mountXtermHost(host, {
      create: () => {
    ...
    return () => {
      if (hostLifecycleRef.current === lifecycle) hostLifecycleRef.current = null;
      lifecycle.dispose();
    };
  }, []);
