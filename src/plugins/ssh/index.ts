...
export const sshPlugin = definePlugin({
  ...
  locales: {
    "zh-CN": {
      ...
      "hostTrustStoreBlockedTitle": "SSH 主机信任库不可用",
      "hostTrustStoreResetPrompt": "TauTerm 已阻止使用当前 SSH 主机信任库：\n{{reason}}\n\n为避免把损坏或不兼容的信任数据静默当作首次连接，SSH 连接保持拒绝。确认将清空当前信任库；之后所有主机都需要重新核对指纹。",
      "hostTrustStoreResetDone": "SSH 主机信任库已重置，请重新连接并核对主机指纹。",
    ...
    "en-US": {
      ...
      "hostTrustStoreBlockedTitle": "SSH Host Trust Store Unavailable",
      "hostTrustStoreResetPrompt": "TauTerm blocked the current SSH host trust store:\n{{reason}}\n\nSSH remains fail-closed rather than silently treating damaged or incompatible trust data as first use. Confirming clears the trust store; every host must be verified again afterward.",
      "hostTrustStoreResetDone": "SSH host trust store was reset. Reconnect and verify host fingerprints again.",
