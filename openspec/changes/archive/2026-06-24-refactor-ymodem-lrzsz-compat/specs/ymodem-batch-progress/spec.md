...
The backend SHALL emit progress events with batch context fields (`file_index`, `total_files`, `aggregate_bytes_transferred`, `aggregate_total_bytes`) and file-boundary events (`transfer-file-start`, `transfer-file-complete`) during multi-file YModem transfers. The backend SHALL compute `aggregate_total_bytes` before starting file transmission and SHALL correct this value (by subtracting the failed file's size) when a file fails to open, ensuring aggregate progress display remains consistent.
...
#### Scenario: Aggregate total corrected on file open failure
- **WHEN** a file in a batch fails to open (e.g., file not found, permission denied)
- **THEN** the `aggregate_total_bytes` SHALL be reduced by that file's size BEFORE any subsequent progress events are emitted, and the file SHALL be recorded as `failed` in batch results with `size: 0`
