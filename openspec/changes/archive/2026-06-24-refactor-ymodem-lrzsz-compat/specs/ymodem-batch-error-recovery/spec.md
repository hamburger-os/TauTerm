...
The YModem sender SHALL continue to the next file when a single file fails after exhausting retries, rather than aborting the entire batch. Failed files SHALL be reported individually with their error reason. When a file fails, the sender SHALL send the lrzsz-standard cancel sequence (10 CAN bytes + 8 backspace bytes) to synchronize with the receiver before proceeding to the next file.
...
- **THEN** the sender MUST log the failure with filename and error reason, send the 10-CAN+8-BS cancel sequence, drain residual RX bytes, skip to the next file in the batch, and continue the transfer
...
- **THEN** the sender MUST still send the batch-end empty block 0 to properly terminate the YModem session, regardless of prior file failures; the sender MUST NOT wait for ACK after sending the empty block 0

#### Scenario: Cancel still aborts entire batch
- **WHEN** the user cancels a batch transfer mid-progress
- **THEN** the sender MUST immediately abort (not skip to next file), send the 10-CAN+8-BS cancel sequence, and return a cancellation error
