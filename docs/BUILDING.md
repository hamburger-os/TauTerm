...
| **CMake** | >= 3.20 | `npm run tauri dev` 会完整构建 vendored TCNOpen/TRDP native helper；正式打包同样需要 |
...
TRDP native helper 使用 CMake + MSVC 构建。为了保证每次桌面开发调试运行的都是完整软件，`npm run tauri dev` 会在启动 Vite/Rust 之前自动重新构建 TRDP helper，因此 Windows 开发环境必须提供 CMake 3.20+ 与可用的 x64 MSVC/Windows SDK 工具链。推荐安装 Visual Studio Build Tools 的 “Desktop development with C++” 工作负荷。当前 Windows 发布目标为 x86_64。
...
如尚未安装 Windows C++ 构建工具，可安装 Build Tools 并加入 C++ 桌面工作负荷：
...
安装或修改工具链后请重新打开 PowerShell，再运行 `cmake --version` 验证。
...
`npm run tauri dev` 会通过 `beforeDevCommand` 自动完整构建 TRDP 原生 helper，然后 `build.rs` 把真实可执行文件放入当前 target triple 的 sidecar 位置。这样每次桌面开发运行都包含可用的 TRDP Node 与 Monitor 实时抓包能力。`npm run trdp:build` 保留为单独诊断/构建原生组件的入口。
...
| 命令 | 用途 |
|------|------|
| `npm run tauri dev` | 先完整构建 TRDP native helper，再启动 Vite + Rust 后端 + 桌面窗口 |
| `npm run dev` | 仅启动 Vite 前端开发服务器 |
| `npm run build` | 执行 TypeScript 检查并构建前端生产资源 |
| `npm run preview` | 本地预览已经构建的前端资源 |
| `npm run trdp:build` | 单独构建/诊断 TRDP native helper 与 reference peer；`tauri dev` 会自动执行同一构建流程 |
...
# 启动完整开发模式；会先自动构建 TRDP native helper
npm run tauri dev

# 如需单独诊断 TRDP 原生构建，可运行
npm run trdp:build
```
