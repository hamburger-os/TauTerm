...
SystemView 是 RTT 上层语义观察器，不属于 RTT backend。Runtime 根据 Channel metadata 中的 `SysView` / `SystemView` 名称自动挂载，也提供显式 attach 命令用于 metadata 不完整或自定义命名场景。每个 SystemView observer：
