...
Network Debug 的目标选择由 SessionContext 拥有；`TargetBar` 只负责展示和选择，独立的目标同步桥接负责把 TCP/UDP server 当前目标传给后端脚本引擎并显式报告同步失败。因此人工发送、命令面板、自动回复和脚本仍使用同一个目标语义，但展示组件本身不再维护第二套运行时副作用。
...
`npm run check:sendbar` 验证基础发送 payload、重复发送背压、导入 schema、会话态/工程资产边界、执行锁和公共确认弹窗等关键合同，并作为 CI 的前端 hygiene 检查之一。
