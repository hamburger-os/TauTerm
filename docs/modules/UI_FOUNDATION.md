...
- Session 列表与 Pane Header 采用统一的双层身份模型：第一行 `Session.name` 是稳定、可显式重命名的会话身份，默认名称仅在创建时计算一次；第二行是当前配置摘要，允许随 host/port/串口/角色等参数变化。协议默认名与摘要由 `PluginRegistration.sessionPresentation` 声明；需要宿主能力计算默认名时使用 `resolveDefaultSessionName`，应用壳不维护 built-in 协议格式。新建会话配置页的名称输入在用户留空时保持“使用自动默认名”的语义，并以 placeholder 实时预览当前参数会生成的默认名称；异步默认名与最终保存必须复用同一 Registry resolver，参数变化后的过期解析结果不能覆盖当前预览。新建会话卡片与配置窗口标题仍统一使用 `PluginManifest.name` 作为 Session 类型显示名；`description` 只描述能力，不承担类型身份；
- 前端只有一个运行时 `PluginRegistry`。协议的 presentation、运行态、发送目标、共享 surface 的运行态可见性、应用覆盖层和右侧栏面板都由 registration contribution 声明；`App.tsx` / `SessionRightSidebar` 只挂载这些 contribution，不识别 SSH、Network、RTT、Local Shell 等具体插件 ID；
...
- 所有 xterm 宿主（公共 Terminal renderer、RTT Terminal 以及未来终端式协议视图）统一复用 `src/components/Terminal/xtermLifecycle.ts`。实例只在宿主节点仍连接到文档且具有非零可测量尺寸后 `open`；初始 fit、Pane/窗口 resize、字体变化、Pane 重新激活和 imperative `fit()` 全部进入同一个 RAF 合并调度器；ResizeObserver 只负责请求调度，不直接同步调用 FitAddon。cleanup 必须先取消待执行 RAF、断开 observer/listener，再释放 xterm，确保 React StrictMode 的开发期 effect 探测、隐藏 Pane、协议视图切换和快速卸载不会让已销毁 renderer 继续读取 dimensions；
...
- 所有声明 `connection` capability 的插件必须同时注册 `sessionPresentation.defaultName`，使新建配置页能在不依赖协议分支的情况下立即预览创建时默认名称；`resolveDefaultSessionName` 只能用于需要宿主/系统解析的进一步细化，不能成为唯一的默认名来源。
...
- Terminal 的 DOM 生命周期必须先于 xterm renderer 生命周期：禁止在隐藏/零尺寸容器上 `open()`，禁止在 ResizeObserver、设置 effect 或 imperative API 中绕过共享 `xtermLifecycle` 直接调用 `FitAddon.fit()`，协议插件不得复制一套近似但不同的 open/fit/dispose 逻辑，也禁止通过关闭 React StrictMode 掩盖 effect cleanup 竞态。
...
- `src/components/Terminal/xtermLifecycle.ts`
