...
- 所有终端粘贴入口统一经 xterm `paste()`，多条实际内容行先进入安全确认预览，右键复制/粘贴/全选/清屏完成后恢复终端焦点；
- 通用组件与图标系统供协议模块复用；
...
- Terminal 的控制键语义与应用快捷键必须显式分层：普通 `Ctrl+C / Ctrl+V` 不应被通用 WebView 剪贴板逻辑隐式劫持；应用级复制/粘贴必须由 TauTerm 宿主明确路由。
- 终端粘贴不能绕过 xterm 直接调用 Session `onData`；安全确认只针对至少两行非空内容，避免普通单行粘贴产生不必要摩擦。
