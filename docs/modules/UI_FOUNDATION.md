...
- Session 列表与 Pane Header 采用统一的双层身份模型：第一行 `Session.name` 是稳定、可显式重命名的会话身份，默认名称仅在创建时计算一次；第二行是当前配置摘要，允许随 host/port/串口/角色等参数变化。协议默认名与摘要完全由 `PluginRegistration.sessionPresentation` 声明，应用壳只消费结果，不维护 SSH、TRDP、Network 等 built-in special case；
- 前端只有一个运行时 `PluginRegistry`。`src/core/plugin-contracts.ts` 只定义无 React/i18n/SessionContext 依赖的共享类型，不镜像注册数据；插件 presentation、状态栏、连接配置、持久化参数投影和重连前置策略都挂在同一个 registration 上；
...
- 可理解且可撤销的专业配置风险优先使用就地非阻塞提示；例如 TFTP 的“非回环监听 + 允许写入 + 允许覆盖”只显示行内 warning，不占用 `ConfirmDialog`；重连时的 fail-closed 检查由 TFTP 插件自己的 `reconnectGuard` 提供，公共 SessionContext 不解析这些字段；
...
- 底边 `StatusBar` 是应用唯一常驻状态带：左侧表达当前活跃 Session 的运行上下文，右侧保留版本、更新器等应用级状态；协议专属运行信息通过 `PluginRegistration.statusBarItems` 注入，而不是继续把协议判断写进全局组件；`StatusBarContext` 只暴露 `sessionId/pluginId/state/endpoint/params` 等公共 Session 快照，Serial 虚拟串口等插件私有运行态由插件 renderer 自己从对应 store/hook 读取；
...
插件 `statusBarItems` 必须保持低交互、低噪声：它们用于快速确认运行态，复杂编辑仍回到对应 Session 工作区。StatusBar 本身仍由应用壳统一拥有表面和布局，插件只提供内容节点。插件需要私有运行态时由自己的 renderer/hook 查询，不通过扩大公共 StatusBar Tab DTO 解决。
...
- 会话配置更新、连接/重连事件只能刷新动态参数与运行态，不得重新生成已存在根会话的 `Session.name`；名称变化必须来自显式重命名或编辑名称字段。协议若需要不同的默认身份或第二行摘要，应扩展自己的 `sessionPresentation`，而不是在 `SessionSidebar`、`SplitView` 或 `sessionPresentation.ts` 中增加协议分支。
...
- 插件专属参数持久化投影与重连前置规则必须由 registration contribution 拥有；公共 `SessionContext` 不得按具体 `pluginId` 清洗密码、解释 TFTP 暴露条件或继续增加其它协议字段判断。
...
- 公共插件 contract 不得为某一个协议新增私有运行态字段；如果某插件需要额外状态，优先建立插件内 Session store/hook 或专用 contribution，而不是把字段塞进 `TabInfo` 的公共投影再一路透传到 registry。
...
修改应用壳、内容适配模型、renderer 类型/实例身份、Terminal/xterm 生命周期、设置架构、i18n 组织、共享选择控件、插件 locale/status-bar/session-presentation/持久化/重连扩展、底边状态栏信息职责、快捷键/命令注册或公共 UI 所有权时，必须同步更新本文。
