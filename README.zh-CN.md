...
对于 TFTP 与 iPerf，Session 工作区本身可在**未连接**状态使用：客户端 GET/PUT 或测速任务属于瞬态操作；“连接”只启动该 Session 的 TFTP/iPerf 服务端，“断开”停止服务端但不隐藏客户端工作台。TFTP 若在非回环接口上同时开启远程写入和覆盖，配置页必须显式确认风险后才允许启动服务端。
