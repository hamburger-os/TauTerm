...
mod tests {
    ...
    fn test_crc16_ccitt_known_vector() {
        ...
        assert_eq!(
            table_result, bitwise_result,
            "查表法 ({:#06X}) 与位计算法 ({:#06X}) 不一致",
            table_result, bitwise_result
        );
    ...
    fn test_crc16_feedthrough_verify_corrupt_data() {
        ...
        corrupt[5] ^= 0x01;
        assert!(
            !crc16_ccitt_feedthrough_verify(&corrupt, crc_hi, crc_lo),
            "数据损坏应导致验证失败"
        );
    }
    ...
    fn test_crc16_feedthrough_verify_corrupt_crc() {
        ...
        // 翻转 CRC 低字节的一位
        assert!(
            !crc16_ccitt_feedthrough_verify(data, crc_hi, crc_lo ^ 0x01),
            "CRC 损坏应导致验证失败"
        );
    }
}
