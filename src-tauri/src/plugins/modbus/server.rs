...
fn process_serial_frame(
    ...
    let request = match codec::decode_request(&pdu) {
        ...
        Err(_) => {
            if !broadcast {
                let response = exception_pdu(pdu.first().copied().unwrap_or(0), 0x03);
                send_serial(handle, config, unit, &response);
            }
            return;
        }
    };
