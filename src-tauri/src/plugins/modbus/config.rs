...
fn default_role() -> ModbusRole { ModbusRole::Client }
fn default_host() -> String { "127.0.0.1".into() }
fn default_tcp_port() -> u16 { 502 }
fn default_unit_id() -> u8 { 1 }
fn default_response_timeout_ms() -> u64 { 1000 }
fn default_read_retries() -> u8 { 1 }
fn default_server_max_clients() -> usize { 16 }
...
impl ModbusConfig {
    pub fn validate(&self) -> Result<(), String> {
        ...
        match self.mode {
            ModbusMode::Rtu | ModbusMode::Ascii => {
                if self.serial_port.trim().is_empty() { return Err("serial_port is required".into()); }
                if self.unit_id > 247 { return Err("serial unit_id must be 0..=247".into()); }
        ...
        if self.server_max_clients > 256 { return Err("server_max_clients must be <= 256".into()); }
        if let Some(code) = self.server_fault.exception_code {
            if !(1..=11).contains(&code) { return Err("fault exception_code must be a standard non-zero exception code".into()); }
        }
        Ok(())
    }
    ...
    pub fn rtu_frame_gap(&self) -> std::time::Duration {
        if self.serial.baud_rate > 19_200 {
            ...
            let micros = (3.5 * bits_per_char * 1_000_000.0 / self.serial.baud_rate.max(1) as f64).ceil() as u64;
