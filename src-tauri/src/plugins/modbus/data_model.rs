        ...
        match request {
            ...
            ModbusRequest::WriteMultipleCoils {
                ...
                for offset in 0..*quantity {
                    ...
                    inner.coils.insert(
                        address.checked_add(offset).ok_or(EX_ILLEGAL_DATA_ADDRESS)?,
                        bit,
                    );
                }
                out.extend_from_slice(&address.to_be_bytes());
...
fn apply_file_writes(
    records: &mut HashMap<(u16, u16), u16>,
    ...
    for request in requests {
        for (offset, value) in request.values.iter().enumerate() {
            let record = request
                ...
                .ok_or(EX_ILLEGAL_DATA_ADDRESS)?;
            records.insert((request.file_number, record), *value);
        }
    }
    Ok(())
}
