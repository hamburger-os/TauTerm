...
pub fn encode(unit_id: u8, pdu: &[u8]) -> Result<Vec<u8>, String> {
    if pdu.is_empty() || pdu.len() > super::pdu::MAX_PDU_LEN { return Err("invalid RTU PDU length".into()); }
...
pub fn decode(frame: &[u8]) -> Result<(u8, Vec<u8>), String> {
    if frame.len() < 4 { return Err("RTU frame too short".into()); }
    let data_len = frame.len()-2;
    let expected = crc16(&frame[..data_len]);
    let actual = u16::from_le_bytes([frame[data_len], frame[data_len+1]]);
    if expected != actual { return Err(format!("CRC mismatch: expected 0x{expected:04X}, got 0x{actual:04X}")); }
