...
pub fn encode(transaction_id:u16, unit_id:u8, pdu:&[u8])->Result<Vec<u8>,String>{
    if pdu.is_empty() || pdu.len()>super::pdu::MAX_PDU_LEN { return Err("invalid TCP PDU length".into()); }
...
pub fn decode(frame:&[u8])->Result<(u16,u8,Vec<u8>),String>{
    if frame.len()<MBAP_LEN+1 { return Err("TCP ADU too short".into()); }
    let transaction_id=u16::from_be_bytes([frame[0],frame[1]]);
    let protocol_id=u16::from_be_bytes([frame[2],frame[3]]);
    if protocol_id!=0 { return Err(format!("invalid Protocol Identifier {protocol_id}")); }
    let length=u16::from_be_bytes([frame[4],frame[5]]) as usize;
    if !(2..=254).contains(&length) { return Err(format!("invalid MBAP length {length}")); }
    if frame.len()!=6+length { return Err(format!("MBAP length mismatch: header={length}, actual={}",frame.len()-6)); }
...
#[derive(Default)]
pub struct TcpFramer { buffer: Vec<u8> }
...
impl TcpFramer {
    pub fn push(&mut self,data:&[u8])->Result<Vec<Vec<u8>>,String>{
        ...
        loop {
            if self.buffer.len()<MBAP_LEN { break; }
...
mod tests {
    ...
    fn fragmented_and_coalesced_frames_are_split() {
        ...
        let mut tail=a[4..].to_vec(); tail.extend_from_slice(&b);
