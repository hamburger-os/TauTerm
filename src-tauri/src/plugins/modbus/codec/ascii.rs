...
pub fn encode(unit_id: u8, pdu: &[u8]) -> Result<Vec<u8>, String> {
    if pdu.is_empty() || pdu.len() > super::pdu::MAX_PDU_LEN { return Err("invalid ASCII PDU length".into()); }
...
pub fn decode(frame: &[u8]) -> Result<(u8, Vec<u8>), String> {
    ...
    if hex.len() % 2 != 0 { return Err("ASCII frame contains odd hex digit count".into()); }
    let mut raw = Vec::with_capacity(hex.len()/2);
    for pair in hex.chunks_exact(2) {
        raw.push((hex_nibble(pair[0])? << 4) | hex_nibble(pair[1])?);
    }
    if raw.len() < 3 { return Err("ASCII frame payload too short".into()); }
    let actual = raw.pop().expect("length checked");
    let expected = lrc(&raw);
    if actual != expected { return Err(format!("LRC mismatch: expected 0x{expected:02X}, got 0x{actual:02X}")); }
    let unit = raw[0];
...
mod tests {
    ...
    fn ascii_round_trip_and_lrc_validation() {
        ...
        let mut broken=frame; broken[3]=b'F';
