...
/// Quiet-mode console suppression (#TauTerm fork): when active, the human
/// report lines skip their stdout/stderr prints while `log::info!` and the
/// `--get-server-output` capture still fire — hosts with their own logging
/// (TauTerm) keep full data in their log files without console noise.
///
/// Process-global like `OUTPUT_TITLE`, but with SAVE/RESTORE guard semantics
/// (not clear-on-drop): concurrent quiet runs in one process (TauTerm's
/// self-test runs client and server simultaneously) compose correctly via the
/// previous-value stack.
static OUTPUT_QUIET: AtomicBool = AtomicBool::new(false);

/// Whether console printing is currently suppressed.
pub(crate) fn output_quiet() -> bool {
    OUTPUT_QUIET.load(Ordering::Relaxed)
}

/// RAII switch for quiet mode: sets the global for the enclosing run scope and
/// restores the previous value on drop, so a quiet run inside a loud process
/// (or vice versa) cannot leak either way — even on early `?` returns.
pub(crate) struct OutputQuietGuard {
    prev: bool,
}

impl OutputQuietGuard {
    pub(crate) fn set(quiet: bool) -> Self {
        let prev = OUTPUT_QUIET.swap(quiet, Ordering::Relaxed);
        OutputQuietGuard { prev }
    }
}

impl Drop for OutputQuietGuard {
    fn drop(&mut self) {
        OUTPUT_QUIET.store(self.prev, Ordering::Relaxed);
    }
}
    ...
    ($($arg:tt)*) => {
        {
            ...
            // #TauTerm fork: quiet mode keeps the log/capture sinks, skips
            // only the console print.
