...
impl StreamCounters {
    ...
    /// [TauTerm fork] All callers are `#[cfg(unix)]` (TCP_INFO path) or tests;
    /// gate identically so non-unix builds stop warning (getters stay
    /// unconditional — they fall back to the unset defaults).
    #[cfg(any(unix, test))]
