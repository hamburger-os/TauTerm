...
fn titled(line: std::fmt::Arguments) {
    ...
    // #TauTerm fork: quiet mode keeps the capture sink, skips the console.
...
/// [TauTerm fork] Live interval channel sender, newtyped so `Client`'s /
/// `Server`'s PartialEq derive keeps working (std mpsc `Sender` has no
/// `PartialEq`): only PRESENCE is part of a config comparison — same
/// convention as `InterruptWatch` (#210).
#[derive(Clone, Debug)]
pub(crate) struct IntervalSender(pub std::sync::mpsc::Sender<crate::json_report::Interval>);

impl PartialEq for IntervalSender {
    fn eq(&self, _other: &Self) -> bool {
        true
    }
}

// The mpsc sender's queue internals are !RefUnwindSafe; the fork only ever
// `send`s on it (a best-effort push, no cross-unwind invariant), so the
// unwinding markers hold — same reasoning as InterruptWatch (#210).
impl std::panic::UnwindSafe for IntervalSender {}
impl std::panic::RefUnwindSafe for IntervalSender {}
...
pub struct IntervalReporterConfig {
    ...
    pub is_server: bool,
    /// [TauTerm fork] Live interval channel: when set, every completed interval
    /// is pushed here in real time, independent of `json_stream` / `print`.
    /// Hosts receive per-second results without waiting for `run()` to return.
    pub interval_tx: Option<IntervalSender>,
}
...
pub fn spawn_interval_reporter(
    ...
    // [TauTerm fork] The live interval channel also needs the typed Interval
    // construction per tick — a host config without -J/--json-stream passes
    // no collector, so gate on the channel too (the accumulated `collected`
    // vec is simply dropped at loop end when no collector exists).
    let collecting = collector.is_some() || config.interval_tx.is_some();
        ...
        let mut emit_interval = |start: f64,
            ...
            if do_emit {
                ...
                if collecting {
                    ...
                    // [TauTerm fork] Live interval channel: hosts get each
                    // interval as it completes (best-effort; a dropped consumer
                    // never stalls the reporter).
                    if let Some(tx) = &config.interval_tx {
                        let _ = tx.0.send(interval.clone());
                    }
