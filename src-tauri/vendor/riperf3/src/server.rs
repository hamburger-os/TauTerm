...
pub struct Server {
    ...
    /// [TauTerm fork] Quiet mode: suppress console stdout/stderr during the
    /// run; `log` sinks and the --get-server-output capture still fire.
...
impl Server {
    ...
    fn banner_line(line: &str) {
        // #TauTerm fork: suppressed in quiet mode (the guard is active for
        // the whole run, banners included).
    ...
    pub async fn run(&self) -> Result<()> {
        // #TauTerm fork: quiet mode suppresses all console prints for this
        // run (save/restore — composes with concurrent runs in-process).
        let _quiet_guard = crate::macros::OutputQuietGuard::set(self.quiet);
        ...
        loop {
            match self.handle_one_test(&listener).await {
                ...
                Err(RiperfError::ClientTerminated) => {
                    ...
                    if !json {
                        // #TauTerm fork: quiet mode suppresses the stderr line.
                        if !crate::macros::output_quiet() {
    ...
    pub async fn run_once(&self) -> Result<crate::json_report::Report> {
        // #TauTerm fork: quiet mode suppresses all console prints for this
        // round (TauTerm drives run_once directly; nesting under run()
        // composes via save/restore).
        let _quiet_guard = crate::macros::OutputQuietGuard::set(self.quiet);
        let listener = self.bind().await?;
        self.run_once_with_listener(&listener).await
    }

    /// #TauTerm fork: bind the listening socket without serving a test.
    ///
    /// Lets the host emit its `running` status only after a successful bind
    /// (no "first green then red" flicker on port-conflict failures) and reuse
    /// the same listener across `run_once_with_listener` rounds (no per-round
    /// rebind window).
    ...
    /// #TauTerm fork: serve one test on an already-bound listener.
    pub async fn run_once_with_listener(
        &self,
    ...
    async fn handle_one_test(
        ...
        // [TauTerm fork] The live interval channel also attaches the collector:
        // the final Report must carry the same intervals the channel streamed
        // (authoritative parity for hosts that reconcile at test end).
        let want_collector = collect_intervals || self.json_stream || self.interval_tx.is_some();
        ...
        let interval_handle = {
            ...
            crate::reporter::spawn_interval_reporter(
                crate::reporter::IntervalReporterConfig {
                    ...
                    bidir: cfg.bidir,
                    is_server: true,
                    interval_tx: self.interval_tx.clone(),
                },
                stream_refs,
                done.clone(),
                reporter_end.clone(),
...
impl ServerBuilder {
    ...
    /// [TauTerm fork] Live interval channel: every completed interval is
    /// pushed to `tx` in real time (best-effort; independent of `json_stream`
    /// and `print`). The final [`crate::json_report::Report`] is unchanged.
    /// The sender spans all `run_once()` rounds of the built [`Server`].
    pub fn interval_channel(
        mut self,
        tx: std::sync::mpsc::Sender<crate::json_report::Interval>,
    ) -> Self {
        self.interval_tx = Some(crate::reporter::IntervalSender(tx));
        self
    }

    /// [TauTerm fork] Quiet mode: suppress console stdout/stderr during the
    /// run; `log` sinks and the --get-server-output capture still receive
    /// every line. Default false.
