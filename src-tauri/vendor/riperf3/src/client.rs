...
impl Client {
            ...
            match state {
                    ...
                    match event {
                        ...
                        Some(ControlEvent::ServerError) => {
                            ...
                            if self.json_output || self.json_stream {
                                ...
                                // #TauTerm fork: suppressed in quiet mode —
                                // the error rides the returned Result.
                ...
                TestState::ServerError => {
                    ...
                    if self.json_output || self.json_stream {
                        ...
                        // #TauTerm fork: suppressed in quiet mode — the
                        // error rides the returned Result.
                        if !crate::macros::output_quiet() {
                            eprintln!("riperf3: SERVER ERROR - {msg}");
                        }
                    }
    ...
    fn print_server_output(&self, server_results: Option<&TestResultsJson>) {
        ...
        if let Some(text) = &server.server_output_text {
            crate::vprintln!("\nServer output:");
            // #TauTerm fork: quiet mode suppresses the raw blob print.
            if !crate::macros::output_quiet() {
                println!("{text}");
            }
    ...
    fn print_results_json(
        ...
        // #TauTerm fork: quiet mode suppresses the -J blob print.
        if !crate::macros::output_quiet() {
    ...
    fn emit_json_stream_end(
        ...
        if self.json_stream_full_output {
            // #TauTerm fork: quiet mode suppresses the blob print.
            if !crate::macros::output_quiet() {
                println!("{}", serde_json::to_string_pretty(&report).unwrap());
            }
        }
