...
Project skills for AI coding agents live in the cross-tool standard location [`.agents/skills/`](.agents/skills/) (com0com driver reference, docs maintenance, theme writing, theme review). Tools that follow the [Agent Skills standard](https://agentskills.io/) — Codex, Cursor, Copilot, Gemini CLI, Windsurf — read them directly. Trae reads them after enabling the ".agents directory" option in its Skills settings.

Claude Code only reads `.claude/skills/`, so that directory contains **full mirrors** of the canonical skills (SKILL.md + `references/` subdirs) generated by `npm run skills:sync`. After editing a skill in `.agents/skills/`, rerun `npm run skills:sync` — the `.claude/skills/` mirrors must stay byte-identical to canonical; CI fails on drift. Never edit `.claude/skills/` directly.
