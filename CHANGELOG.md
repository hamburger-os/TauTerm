...
### Fixed
- **RTT attach failures were missing from the System Log** — RTT now records connection configuration, native probe/target readiness, locator provenance (including resolved `_SEGGER_RTT` addresses and RAM-scan fallback), successful Control Block/channel metadata, refresh failures and runtime faults. Corrupted Control Block failures also emit a bounded descriptor snapshot and compare block-vs-word memory reads so invalid secondary channels can be distinguished from probe transport read inconsistencies. Technical detail is retained while TauTerm's RTT semantics are presented first.
