...
### Fixed
- **RTT SystemView could keep recovered presentation drops as a permanent warning, obscure task rows under a translucent sticky header, clip the last timeline tick, and race xterm disposal when switching views** — the trace UI now distinguishes pending versus recovered presentation gaps, reports current target-loss rate and observed coverage, suppresses pseudo-precise CPU percentages when target loss dominates, separates the task header from scrolling rows, clamps timeline labels to the canvas, formats raw SystemView descriptions for display, and shares one guarded xterm open/fit/dispose lifecycle with the common Terminal renderer.
