...
### Changed
- **Visual performance tiers, system-motion visibility and shared four-color ambient** — all tiers retain the complete Red/Yellow/Green/Blue field. Quality now animates both diagonal ambient layers independently over wider, overlap-capable transform paths with stronger small-glass sampling; Balanced keeps both color layers but animates only one and lowers small-glass sampling so its continuous compositor workload is structurally lower instead of merely slower; Compatibility keeps both layers static and uses the flat translucent fallback. Appearance settings now report the live OS reduced-motion preference so a disabled Windows animation setting no longer makes Quality/Balanced appear mysteriously broken. All three themes still share the same ambient geometry/strength and differ through their neutral/black/white material bases.
