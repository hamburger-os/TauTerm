...
- **Split Workspace surface ownership and resize fluency** — the center Workspace now owns one shared Content glass border/shadow/radius instead of wrapping every Pane/terminal in another full Content frame; Pane state is expressed with lightweight active/inactive veils and Header/divider separators. Pane-local corner calculations and geometry transitions were removed so split dragging follows the pointer directly with less layout/paint work.
...
### Fixed
- **Split View context-menu targeting** — the Pane-level `Close Pane` menu is now header-only: connected Network Debug/custom content and empty Pane bodies no longer open it. Right-clicking a non-selected Pane Header no longer activates that Session on mouse-down, preventing SendBar/RightSidebar reflow from moving the title before `contextmenu` resolves.
