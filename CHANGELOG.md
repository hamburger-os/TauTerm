...
### Changed
- **SFTP file-manager safety and task model** — file transfers now return an exact `transfer_id` at command acceptance, cancellation targets that task, directory waits no longer use a fixed five-minute wall-clock timeout, Save As preserves the chosen filename, directory queries reject stale responses, recursive downloads preserve empty directories and do not follow symbolic links by default, and upload conflicts support Replace / Keep Both / Skip Existing.
