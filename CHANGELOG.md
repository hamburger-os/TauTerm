...
- **Windows virtual-port bridge ownership** — replaces cloned reader/writer COM workers with one serialized Endpoint Actor and one `FILE_FLAG_OVERLAPPED` bridge handle per external endpoint. Peer/RX/error changes use `WaitCommEvent`, read/write use cancellable overlapped I/O, transient status failures remain distinct from confirmed data gaps, and shutdown drains `CancelIoEx` before joining every handle owner.
- **Release boundary enforcement** — release validation now shares the same CHANGELOG parser as GitHub Release-note generation, rejects a non-empty `Unreleased` section for the target version, and keeps regression coverage for version-section extraction so post-freeze changes cannot silently ship without release notes.
...
### Fixed
- **Process-level failures could leave no useful TauTerm evidence** — installs bounded local panic/crash diagnostics, including Rust backtraces and Windows normal minidumps generated by a pre-started out-of-process helper while preserving the normal Windows/WER crash path. Diagnostic exports report only crash-capture health metadata and never copy or upload dump contents.
