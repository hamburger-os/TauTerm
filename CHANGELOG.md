...
### Changed
- **Visual performance tiers and shared four-color ambient** — makes Quality / Balanced / Compatibility a real monotonic cost ladder: all three retain the complete Red/Yellow/Green/Blue ambient field, Quality and Balanced now use much wider transform-only travel without enlarging the ambient raster layer so motion is easier to perceive at essentially the same compositing cost, and Compatibility freezes both ambient layers and reduces the major glass surfaces to one flat translucent fill with no backdrop sampling, layered specular backgrounds, or large glass shadows. All three themes consume the same ambient geometry/strength and clear-glass physics outside the compatibility fallback; Obsidian and Frosted remain black/white base variants rather than separate color fields.
