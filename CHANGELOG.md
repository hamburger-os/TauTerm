...
### Fixed
- **Crash-safe local state persistence** — ConfigStore, SSH known-host trust and the Session Library now commit through atomic file replacement. Session Library read-modify-write paths fail closed on read errors instead of treating an unreadable library as empty, and a failed durable session deletion no longer disappears only from the current UI.
- **Overload-safe logging path** — disabling Session Data Log now stops producers before the shared queue, presentation overflow warnings use the same exponential throttling as UI notices, and system-log file failures cannot recursively feed the LogBridge; write failures are counted as explicit log loss.
