...
### Changed
- **Embedded observability foundation** — promotes native debug-probe ownership into a process-local shared target registry with one worker-owned probe-rs Session per connection identity, bounded operation scheduling and service leases. RTT now consumes that shared target runtime instead of owning the probe directly, while preserving independent Up/Down selection, canonical acquisition ordering and separate history/automation/presentation loss semantics; frontend RTT history is bounded both per Channel and per Session.
