...
### Changed
- **Windows virtual-port bridge ownership** — replaces cloned reader/writer COM workers with one serialized Endpoint Actor and one bridge handle per external endpoint, keeps transient peer-status failures distinct from confirmed data gaps, and makes shutdown cancel blocking Windows I/O before joining every handle owner.

### Fixed
- **Process-level failures could leave no useful TauTerm evidence** — installs bounded local panic/crash diagnostics, including Rust backtraces and Windows normal minidumps while preserving the normal Windows/WER crash path. Diagnostic exports report only crash-capture health metadata and never copy or upload dump contents.
- **Virtual-port health copy was incomplete** — adds localized degraded/backpressure/bridge-failure status text so failure states no longer fall through to raw i18n keys.
...
### Fixed
- **SystemView trace health could overstate UI loss and hide an actively overflowing target** — the trace view now separates recoverable presentation-queue drops from unrecovered sequence gaps, backfills recoverable gaps from bounded backend history, reports recent target-loss rate and known-event coverage, exposes the active SysView Up-buffer size in diagnostics, suppresses misleading CPU percentages under severe target loss, clamps timeline labels inside the canvas, and gives the sticky task header an opaque-enough canonical content backing. RTT Terminal also shares the common xterm host lifecycle so rapid Terminal/SysView switching cannot leave a fit callback touching a disposed renderer.
- **SystemView task metadata could remain permanently “syncing” after three retries, and high-rate trace draining slept between every poll** — task metadata recovery now belongs to the Rust SystemView runtime, uses a bounded 10-round backoff that resets when the unresolved task set makes progress, and exposes its real recovery state to the UI. RTT now keeps draining non-empty bounded polls and only applies the configured sleep while idle, trace integrity separates target/decoder/presentation loss, timestamp and CPU clocks are displayed independently, incomplete task percentages are marked as lower bounds, timeline windows seed their entry scheduler state from retained history, and reused task IDs are reset after termination/create.
- **One invalid RTT descriptor could block every healthy Channel, and hidden RTT target rows inflated the SendBar** — native RTT keeps probe-rs strict attach for healthy targets, but can quarantine invalid individual Up/Down directions and continue with validated channels when the Control Block header remains trustworthy. Invalid directions stay visible with diagnostics and are excluded from polling, automation and sending. Send-target visibility is now runtime-aware and shares the same decision with SendBar host geometry, so RTT no longer reserves a target-row height before a usable Down Channel exists.
- **RTT attach failures were missing from the System Log** — RTT now records connection configuration, native probe/target readiness, locator provenance (including resolved `_SEGGER_RTT` addresses and RAM-scan fallback), successful Control Block/channel metadata, refresh failures and runtime faults. Corrupted Control Block failures emit a bounded descriptor snapshot; when an ELF/user Exact address still reports Control Block Not Found, TauTerm records a bounded 16-byte byte/block/word read comparison and RTT-magic match results instead of silently falling back to RAM scanning. Technical detail is retained while TauTerm's RTT semantics are presented first.
- **Remote-document close confirmation** — dirty SFTP documents now use the shared binary confirmation dialog with a clear “Save changes?” prompt; Confirm saves successfully before closing and Cancel returns to the editor, replacing the custom Close / Save / Cancel card and its misleading “Modified” timestamp title. The dirty-state indicator also uses a dedicated unsaved-changes accessibility label instead of reusing the file timestamp label.
- **com0com identifier conflicts could create the wrong bus and later target another pair for cleanup** — the GUI no longer preselects CNCA/CNCB bus IDs before elevation or generates elevated `.cmd` files. Privileged code serializes mutations with a global mutex, runs `setupc --silent`, allocates from authoritative driver state, verifies the actual post-install CNCA/CNCB→COM mapping, and only then commits ownership. The interactive “identifiers already used” dialog is no longer part of the product flow.
- **Virtual-port DataPlane detach errors were ambiguous and too sensitive to small chunks** — subscriptions now expose bounded per-consumer capacity and a recorded termination reason; VPort derives a finite burst capacity from serial line rate while retaining endpoint-local byte budgets, so a burst of many small Windows serial reads no longer hits the generic 256-message ceiling without a useful diagnosis.
...
- **Engineering data workflows** — adds a multi-view Data Inspector plus serial frame timing, Unix/HEX timestamp conversion, IPv4/CIDR calculation and byte-level diffing. Quick-tool panels remain mounted so switching between checksum, encoding, bit/layout, data and engineering views preserves current working input.

### Changed
- **File-manager conflict decisions** — upload conflicts now present Replace / Keep Both / Skip Existing as a dedicated decision group with clear destructive/recommended/secondary hierarchy, while Cancel is separated into the dialog footer instead of appearing as a fourth equal option.
- **SFTP file-manager safety and task model** — file transfers now return an exact `transfer_id` at command acceptance, cancellation targets that task, directory waits no longer use a fixed five-minute wall-clock timeout, Save As preserves the chosen filename, directory queries reject stale responses, recursive upload/download preserves empty directories and does not follow symbolic links by default, and upload conflicts support Replace / Keep Both / Skip Existing.
- **Large-directory and preview workflows** — file list/grid rendering now windows large directories instead of mounting every row/tile, grid keyboard focus uses roving tabindex with standard navigation keys, and remote preview works from bounded raw bytes with Text/HEX views plus selectable engineering encodings instead of assuming UTF-8.
- **Native file-manager drag/drop** — desktop files and folders can be dropped directly onto the SFTP file-manager panel through Tauri's native drag/drop event stream; DPI-aware hit testing prevents drops over the terminal or other panels from being misrouted, and mixed file/folder conflicts remain no-clobber by default.
- **Unified transfer ownership and scheduling** — `TransferContext` is now the single frontend started/progress/finished listener and stores per-Session transfer snapshots; the SFTP compact status bar projects that store instead of running a second state machine. Backend Inline and SideChannel admission/cancellation now share a Session-level `TransferScheduler` with an explicit default concurrency of one, replacing parallel ad-hoc transfer state fields.
- **File-manager destructive-action UX** — delete now uses a themed accessible confirmation dialog with single-file/directory/batch-specific wording and safe default focus; Properties is the final item in single-file and directory context menus.
- **Quick-tool navigation** — the narrow right-sidebar category switcher now uses the same themed Select control as Protocol Inspector instead of a horizontally scrolling tab strip, eliminating native horizontal-scrollbar artifacts while preserving all mounted tool state.
...
- **Unified binary confirmation dialogs** — destructive or risky yes/no flows now share one accessible `ConfirmDialog` implementation with consistent Liquid Glass geometry, focus trapping/restoration, Cancel/Confirm semantics and theme behavior. Multi-choice business decisions such as file conflicts remain separate instead of being forced into a binary dialog.
- **SendBar architecture hardening** — payload encoding/routing, loop-send backpressure, per-session draft state, shared engineering assets, import validation and execution snapshots now have explicit ownership. Only the active mode mounts its execution UI, edits are locked while a runtime snapshot is executing, and Network Debug target synchronization is separated from presentation controls.
- **Workspace Pane operation model** — Pane Header actions now own clear/split/close operations without disconnecting or deleting Sessions, empty selected panes use explicit nullable Session state, and low-value double-click clear/reset-ratio gestures were removed so Pane operations remain visible and predictable.
- **journald viewer lifecycle** — history paging, full export and realtime streaming now share explicit cursor/cancellation/error semantics; `journalctl` execution is bounded, realtime entries are batched, compact rendering is windowed, arbitrary journald JSON fields are preserved, and stale async responses are rejected.
- **Windows virtual-port ownership model** — TauTerm now separates external user-visible endpoints from internal bridge endpoints, persists privileged-service endpoint ownership, defines orphan cleanup strictly from owned-minus-active resources, makes multi-pair creation transactional, and treats com0com as a shared system driver so pre-existing or third-party resources are never deleted merely because TauTerm can enumerate them.
...
### Fixed
- **Serial endpoint labels and virtual bridge visibility** — serial discovery presents the real system endpoint once instead of repeating trailing `(COMx)` text, while TauTerm-owned internal bridge endpoints stay hidden from the normal port picker and only the external virtual endpoint enters the frontend contract.
- **Network Debug send-target startup errors** — persisted/disconnected TCP/UDP server Sessions no longer synchronize a send target before the runtime side channel exists. Connected-session failures remain visible, while stale async failures after lifecycle or target changes are suppressed.
...
- **Support diagnostics** — Settings → About can export a versioned sanitized diagnostic JSON with build/platform health, plugin metadata, aggregated Session state and log-loss counters. Credentials, endpoints, Session names, raw payloads, System Log contents and Session Data Logs are deliberately excluded; Rust owns the native destination picker.
...
### Fixed
- **Dialog typography and action consistency** — delete confirmation and terminal paste-safety dialogs now share the same Liquid Glass dialog geometry, tokenized title/body/meta typography and public GlassButton variants; destructive delete is a danger action with Cancel focused by default, and conflict-dialog geometry uses the same frame tokens.
- **File-manager modal keyboard semantics** — Properties and Preview now expose proper modal dialog semantics, move focus into the dialog on open, trap Tab/Shift+Tab within available controls, and give icon-only close actions accessible labels.
- **File-manager themed feedback and drag overlay** — native `alert()` calls in file-manager actions now use the global themed Toast path; the drag/drop overlay no longer creates a component-local backdrop blur or references the undefined `--glass-bg` token, and instead reuses the shared control-surface token.
- **Themed native Select popups** — file preview now uses the canonical Liquid Glass select classes, dark themes advertise a dark native `color-scheme` and Frosted advertises light, preventing WebView/OS option popups from rendering as white menus with dark-theme text; the shared select arrow inset is also no longer reset by the padding shorthand.
- **File-manager completion speed visibility** — successful SFTP cards retain the last reliable backend I/O throughput sample through Finalizing/Completed and display it during the five-second auto-dismiss window; normal narrow sidebars keep live speed visible, with speed hidden only at the <=220px emergency layout. Completed-task timestamps also prevent a long-finished card from reappearing when the right sidebar is later remounted.
- **Quick-tool default category** — Quick Tools now initializes from the first canonical tab (`Checksum`) instead of a stale hard-coded `Data Inspector` default that disagreed with the visible category order.
- **SFTP overwrite/cancellation could destroy an existing destination** — uploads and downloads now write same-directory TauTerm temporary files and only commit after write/flush/metadata finalization; Replace uses a backup-and-rollback boundary so cancellation, disconnects, or commit failures cannot truncate/delete the previous final file. New files and upload temp files use exclusive creation, Keep Both/Skip enforce no-clobber at commit time rather than relying on a stale preflight check, and directory Keep Both reserves a distinct root instead of merging into an existing folder.
- **SFTP property chmod could target unsafe entry types** — chmod is now limited to regular files/directories and preserves POSIX file-type bits; symbolic links and special files are excluded from the permission editor.
- **Cross-platform SFTP download names could escape the selected root** — remote-derived local path components are now validated in the backend before joining. Backslashes, parent components, NUL, Windows-reserved characters/device names and other unsafe components are rejected instead of being reinterpreted as local path syntax; directory roots are no longer concatenated from remote names in WebView.
- **SFTP task registration and stale-conflict races** — accepted SideChannel transfers now register a gated task in SessionStore before emitting `started`, so concurrent session close cannot detach an untracked transfer. Uploads whose listing is stale default to Keep Both unless the user explicitly chose Replace, and partial batch-delete failures are surfaced without discarding the user's selection.
- **SFTP transfer progress could show `100% · 0 KB/s` while completion was still pending** — SFTP throughput is now measured at the real async I/O layer with high-resolution timing instead of WebView event-arrival timestamps; duplicate terminal 100% samples are removed, 100% enters an explicit Finalizing state until flush/metadata/batch completion finishes, and completed rows show a semantic Completed state instead of a fake zero speed.
- **File-manager transfer completion ordering and batch outcome races** — every started transfer now has a `transfer_id`; queued progress/file/batch events are drained before `finished`, Session transfer occupancy is released before completion is announced, exact-transfer IDs are used by download waiters, partial batch failures can no longer report success, and cancelled remaining files are accounted for explicitly.
- **File-manager transfer action could be clipped in a narrow right sidebar** — the bottom status bar is now a container-query responsive grid with a fixed cancel/close action column; low-priority speed information yields first, list/grid bottom reservation follows the responsive status height, successful transfers auto-dismiss after 5 seconds (paused while hovered), and failed/cancelled states stay until closed.
...
- **TRDP runtime and state architecture** — native operations now use correlated request/response IPC and a single-owner TCNOpen runtime queue; Session connection waits for the native open result and unexpected helper exit marks the Session disconnected. Workspace import moves to a strict v2 schema with session-level redundancy groups, and object UI states explicitly model Starting/Running/Stopping/Sending/Error instead of inferring success from stdin writes.
- **Performance-aware prism button motion** — high-value four-color Primary and Selected buttons now use a small transform-only flowing color layer in Effects First, while Performance First keeps the same complete four-color treatment fully static. Reduced-motion and layout-resize states stop or pause the decorative button flow, preserving accessibility and the low-cost performance path.
- **Brighter Spectrum Flow identity** — raises the neutral base and lightens the shared glass veils for a clearer luminous look, while Performance First now uses brighter high-alpha smoke-glass fills instead of near-black fallbacks. This keeps the low-cost no-blur path and text readability intact while making Spectrum Flow visibly distinct from Obsidian even when ambient motion is disabled.
- **About page identity cleanup** — removes the duplicated one-line product description from Settings → About. The About page now focuses on product name, version, updates and build metadata, avoiding a second copy of the product positioning that can drift from the repository's canonical project description.
- **Two visual-performance modes and brand-neutral four-color ambient** — removes the low-value middle tier and keeps only Effects First and Performance First. Effects First animates both enlarged diagonal color fields over wide overlap-capable transform paths and keeps richer small-surface glass sampling; Performance First retains the complete four-color field statically and uses the flat translucent fallback with no backdrop sampling or decorative ambient animation. Appearance settings still report the live operating-system reduced-motion preference. Theme/color identifiers and user-facing copy now use TauTerm-owned neutral terminology rather than third-party brand names.
- **TRDP/iperf/TFTP session identity and TRDP usability** — TRDP Monitor now enumerates real Npcap/libpcap capture devices for dropdown selection; new sessions use `TRDP @ Node/Monitor`, `iperf @ iperf2/iperf3`, and `TFTP @ <file root>` default names only at initial creation while later configuration changes preserve the session name; iperf and TFTP sidebar subtitles show their listen endpoints instead of repeating the same value on both card rows; disconnected PD Request objects can be removed without trying to clean an already-destroyed native handle; and TRDP configuration, navigation, common table/inspector labels and actions now follow the active UI language while protocol identifiers/engineering terms remain intact.
...
- **Split Workspace surface ownership and resize fluency** — the center Workspace now owns one shared Content glass border/shadow/radius instead of wrapping every Pane/terminal in another full Content frame; Pane state is expressed with lightweight active/inactive veils and Header/divider separators. Pane-local corner calculations and geometry transitions were removed, selected Headers now use a single 1px accent separator, idle dividers are visually quieter, WebKit scrollbar arrow buttons are hidden, and divider mousemove updates are coalesced to at most one layout commit per animation frame with a final flush on release.
- **Plugin-owned global SendBar capability** — plugin manifests now explicitly declare whether the TauTerm bottom SendBar applies. Serial/SSH/Telnet/Network Debug keep it; Local Shell/TFTP/iperf/TRDP do not. Session configuration can disable a supported bar but cannot enable one for a protocol whose workflow is self-contained.

### Fixed
- **TRDP analysis alignment and table surfaces** — Flow and Packet now share the same Analysis header structure and equal-width columns, with role-specific table width/scroll behavior so Flow no longer gets an unnecessary scrollbar in a wide Pane. Scrollable tables use one wrapper-owned glass surface instead of nested borders, PD/MD filter selectors keep stable geometry on hover/selection, and wide single-pane Overview uses two-column cards to reduce unused vertical space.
- **TRDP tab-strip height and empty-analysis overflow** — the Node 总览/PD/MD/分析 tabs now keep identical fixed geometry even while the selected tab is hovered, instead of inheriting the generic prism-button lift/scale. In shallow 2×2 panes the empty Analysis view also tightens empty-table spacing and defers the packet inspector until a packet is selected, so an idle analysis page no longer needs a root vertical scrollbar.
- **TRDP 2×2 split-pane density** — SplitView now publishes both Pane width and height to custom views and leaves scrolling to the custom Session root, while TRDP uses a shallow-Pane compact density, two-column Overview cards and a two-column Monitor capture form where width permits. This removes the cramped one-column stacking and redundant Pane scrolling visible in four-pane layouts without hiding controls.
- **TRDP review follow-ups** — centralizes saved-session reconnect preflight and restores the kernel as the single `connect_session` router; runtime-only Node/Monitor actions now require a connected Session, SDT status no longer reports detection without evidence, MD Confirm is restricted to ReplyQuery transactions owned by the current Node runtime, capture Flow/sequence/interval/Jitter statistics are aggregated over the complete Rust CaptureStore while packet inspection uses bounded pagination, and narrow Subscriber diagnostics no longer inherit the generic wide-table minimum.
- **Small split panes remain operable** — Split Pane surfaces now publish a CSS container so TFTP, iperf and TRDP adapt to the actual Pane width without per-plugin resize observers. TFTP panels no longer flex-shrink into clipped, unreachable controls in short panes, and narrow custom views stack/scroll at pane-relative breakpoints.
- **Disconnected custom-session context menu** — TFTP, iperf and TRDP content surfaces now expose the same Connect / Configure / Delete right-click workflow as disconnected terminal panes, while the Pane-level Close Pane action remains header-only.

### Protocols
- **TRDP Node + Monitor** — adds a first-party custom TRDP session backed by TCNOpen 3.0.0.0. Node sessions can combine PD Publisher/Subscriber/Request and MD Notify/Request/Listener-Replier roles, including MD UDP/TCP Reply/ReplyQuery/Confirm flows, A/B link selection, topology/redundancy metadata and diagnostics. PD Request/MD Request/MD Notify use one-shot Send semantics; persistent objects use explicit Start/Stop. Subscriber/PD Request timeout is explicit Auto/Custom rather than a hard-coded cycle multiplier. Timeout policy now distinguishes Auto, Custom and Disabled (`TRDP_INFINITE_TIMEOUT`) so official XML timeout=0/omitted semantics are preserved.
- **TRDP XML, Dataset and capture tooling** — parses TCNOpen-style XML with a structured XML parser, preserves BOOL8/BITSET8/ANTIVALENT8 semantic names, maps ComID to Dataset definitions, decodes/encodes structured payload fields while keeping raw HEX as the wire truth source, opens pcap/pcapng, and uses one Rust decoder for live/offline UDP/TCP TRDP including MD/TCP stream reassembly. Live raw frames stay in a bounded Rust CaptureStore, while offline captures are parsed and exported as streams; pcapng export preserves A/B provenance without round-tripping full captures through React. Monitor decodes the MD UUID/status/timeout/URI wire header at the correct offsets, validates TRDP header CRC/protocol compatibility and marks captured messages with incomplete declared payloads protocol-invalid instead of silently treating them as complete. XML import preserves PD/MD telegram classification so MD entries are not auto-created as PD Subscriber templates; arbitrary XML URI strings remain visible in Preview but are not misused as IPv4 Node addresses. Auto live-capture filters follow custom PD/MD ports, Custom remains a raw BPF override, and live-capture save state is committed from the native start ACK so failed/restarted capture attempts do not save stale offline traffic.
...
### Fixed
- **Split View scrollbar corner artifact** — when both horizontal and vertical scrollbars are needed inside an undersized Pane, the WebKit scrollbar intersection is now transparent instead of rendering a bright square at the lower-right corner.
- **Split View context-menu targeting** — the Pane-level `Close Pane` menu is now header-only: connected Network Debug/custom content and empty Pane bodies no longer open it. Right-clicking a non-selected Pane Header or connected terminal no longer activates that Session on mouse-down, preventing SendBar/RightSidebar reflow from moving the interaction target before the intended context menu resolves.
- **Network Debug SendBar while disconnected** — Network Debug now follows the same SendBar visibility rule as other SendBar-enabled Sessions, so selecting a disconnected debugger shows the bottom SendBar immediately; connecting/disconnecting no longer creates or removes the SendBar shell.
...
- Three themes: Spectrum Flow, Obsidian, Frosted (light)
