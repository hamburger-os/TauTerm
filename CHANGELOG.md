...
- **SystemView task metadata could remain permanently “syncing” after three retries, and high-rate trace draining slept between every poll** — task metadata recovery now belongs to the Rust SystemView runtime, uses a bounded 10-round backoff that resets when the unresolved task set makes progress, and exposes its real recovery state to the UI. RTT now drains busy streams in bounded bursts, trace integrity separates target/decoder/presentation loss, timestamp and CPU clocks are displayed independently, incomplete task percentages are marked as lower bounds, timeline windows seed their entry scheduler state from retained history, and reused task IDs are reset after termination/create.
