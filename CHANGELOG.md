...
### Changed
- **Visual performance tiers and shared four-color ambient** — makes Quality / Balanced / Compatibility a real monotonic cost ladder: all three retain the complete Red/Yellow/Green/Blue ambient field, Quality uses stronger small-glass sampling and more visible transform-only drift, Balanced keeps the recommended lower-cost motion, and Compatibility freezes both ambient layers and disables backdrop sampling while using denser fallback veils so it no longer becomes the most transparent mode. All three themes now consume the same ambient geometry/strength and the same clear-glass physics; Obsidian and Frosted differ through black/white base veils rather than their own color-field intensity.
