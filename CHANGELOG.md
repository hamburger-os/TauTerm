...
- **Unified frontend transfer ownership** — `TransferContext` is now the single started/progress/finished listener and stores per-Session transfer snapshots; the SFTP compact status bar projects that store instead of running a second transfer state machine.
