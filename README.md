...
TCNOpen TRDP 3.0.0.0 is vendored as MPL-2.0 source and built into a separate TauTerm sidecar. Windows live capture uses a separately installed Npcap; Linux/macOS use the system libpcap. Offline capture analysis does not require either capture runtime. See the [TRDP session guide](docs/TRDP.md) for the current workflow, implementation boundaries and known limitations.
